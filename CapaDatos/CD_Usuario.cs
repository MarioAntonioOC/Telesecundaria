    ...
    public class CD_Usuario
    {
        public static int LoginUsuario(string Usuario, string Clave)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    Console.WriteLine("Error en LoginUsuario: " + ex.Message);
                    respuesta = 0;
                }
            }
        ...
        public static Usuario ObtenerDetalleUsuario(int IdUsuario)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                try
                {
                    ...
                    using (XmlReader dr = cmd.ExecuteXmlReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            Console.WriteLine(doc.ToString()); // Registro de depuración
                            ...
                            if (doc.Element("Usuario") != null)
                            {
                                ...
                                rptUsuario.oRol = doc.Element("Usuario").Elements("DetalleRol") == null ? new Rol() :
                                    (from dato in doc.Element("Usuario").Elements("DetalleRol")
                                     select new Rol()
                                     {
                                         Descripcion = dato.Element("Descripcion").Value
                                     }).FirstOrDefault();

                                rptUsuario.oListaMenu = doc.Element("Usuario").Element("DetalleMenu") == null ? new List<Menu>() :
                ...
                {
                    Console.WriteLine("Error en ObtenerDetalleUsuario: " + ex.Message);
                    rptUsuario = null;
                }
            }
